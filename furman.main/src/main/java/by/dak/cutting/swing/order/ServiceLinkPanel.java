 ...
 * Таб для фурнитуры
 */
public class ServiceLinkPanel extends BaseTabPanel<OrderItem> {
    private ResourceMap resourceMap = Application.getInstance().getContext().getResourceMap(this.getClass());
    private ServiceLinkSupporter supporter = new ServiceLinkSupporter(this);


    public ServiceLinkPanel() {
        initComponents();
    ...
    private void addActions() {
        ActionMap aMap = Application.getInstance().getContext().getActionMap(ServiceLinkPanel.class, this);
    ...
    private void initComponents() {
        ...
        serviceOrderScrollPane = new javax.swing.JScrollPane();
        setTable(new BaseTable());

        setName("Form"); // NOI18N

        serviceOrderScrollPane.setName("serviceOrderScrollPane"); // NOI18N
        ...
        serviceOrderScrollPane.setViewportView(getTable());
        ...
        layout.setHorizontalGroup(
                layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                        .addGroup(layout.createSequentialGroup()
                                .addContainerGap()
                                .addComponent(serviceOrderScrollPane, javax.swing.GroupLayout.DEFAULT_SIZE, 372, Short.MAX_VALUE)
                                .addContainerGap())
        );
        layout.setVerticalGroup(
                layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                        .addGroup(layout.createSequentialGroup()
                                .addContainerGap()
                                .addComponent(serviceOrderScrollPane, javax.swing.GroupLayout.DEFAULT_SIZE, 271, Short.MAX_VALUE)
                                .addContainerGap())
        );
    ...
    private javax.swing.JScrollPane serviceOrderScrollPane;
    ...
    private void initGuiTable() {
        ...
        typeComboBox.setModel(new DefaultComboBoxModel(new Vector(FacadeContext.getServiceFacade().loadAll())));


        getTable().setDefaultEditor(PriceAware.class, new ComboBoxCellEditor(codeComboBox) {
            @Override
            public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row, int column) {
                Service service = (Service) table.getModel().getValueAt(row, 0);

                if (service != null) {
                    Class<? extends PriceAware> priceAwareClass = service.getServiceType().getMaterialType().priceAwareClass();
                    List types = getFacadeBy(priceAwareClass).loadAll();

                    codeComboBox.setModel(new DefaultComboBoxModel(new Vector(types)));
                }
                return super.getTableCellEditorComponent(table, value, isSelected, row, column);
            }
        });
        ComboBoxCellEditor editor = new ComboBoxCellEditor(typeComboBox);
        getTable().setDefaultEditor(Service.class, editor);
    ...
    private void initBinding() {
        ...
        JTableBinding.ColumnBinding columnBinding = jTableBinding.addColumnBinding(ELProperty.create("${service}"), "service");
        columnBinding.setColumnName(resourceMap.getString("table.column.serviceType"));
        columnBinding.setColumnClass(Service.class);
        columnBinding.addBindingListener(supporter.getClearServiceCodeListener());
        columnBinding.setEditable(true);

        columnBinding = jTableBinding.addColumnBinding(ELProperty.create("${priceAware}"), "priceAware");
        columnBinding.setColumnName(resourceMap.getString("table.column.serviceCode"));
        columnBinding.setColumnClass(PriceAware.class);
        columnBinding.setEditable(true);

        columnBinding = jTableBinding.addColumnBinding(ELProperty.create("${priceAware.unit}"), "unit");
        ...
        jTableBinding.addBindingListener(supporter.getServiceLinkChangedController());
    }
    ...
    protected void valueChanged() {
        supporter.setLinkList(FacadeContext.getServiceLinkFacade().loadAllBy(getValue()));
    }
    ...
    public void onDelete() {
        ...
        if (selRow > -1) {
            ServiceLink serviceLink = supporter.getLinkList().get(selRow);
            if (serviceLink.hasId()) {
                if (MessageBox.confirmYesNoDeletion()) {
                    supporter.getLinkList().remove(serviceLink);
                    FacadeContext.getServiceLinkFacade().delete(serviceLink);
                }
            }
        }
    }
