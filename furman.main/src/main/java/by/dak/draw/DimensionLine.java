...
public class DimensionLine implements Serializable
{
    ...
    public DimensionLine(Point2D startP, Point2D endP, double offset)
    {
        this(startP.getX(), startP.getY(), endP.getX(), endP.getY(), offset);
    }
     ...
     */
    public DimensionLine(double xStart, double yStart, double xEnd, double yEnd, double offset)
    {
        this.xStart = xStart;
        this.yStart = yStart;
        this.xEnd = xEnd;
    ...
    public double[][] getPoints()
    {
        ...
        double dx = (float) -Math.sin(angle) * this.offset;
        double dy = (float) Math.cos(angle) * this.offset;

        return new double[][]{{this.xStart, this.yStart},
    ...
    public boolean isMiddlePointAt(float x, float y, float margin)
    {
        ...
        double dx = (float) -Math.sin(angle) * this.offset;
        double dy = (float) Math.cos(angle) * this.offset;
        double xMiddle = (xStart + xEnd) / 2 + dx;
        double yMiddle = (yStart + yEnd) / 2 + dy;
    ...
    {
        double angle = Math.atan2(this.yEnd - this.yStart, this.xEnd - this.xStart);
        Line2D startExtensionLine = new Line2D.Double(this.xStart, this.yStart,
    ...
    {
        double angle = Math.atan2(this.yEnd - this.yStart, this.xEnd - this.xStart);
        Line2D endExtensionLine = new Line2D.Double(this.xEnd, this.yEnd,
    ...
    private Shape getShape()
    {
        if (this.shapeCache == null)
        {
            ...
            dimensionLineShape.append(new Line2D.Double(this.xStart + dx, this.yStart + dy, this.xEnd + dx, this.yEnd + dy), false);
            // Append extension lines
            dimensionLineShape.append(new Line2D.Double(this.xStart, this.yStart, this.xStart + dx, this.yStart + dy), false);
            dimensionLineShape.append(new Line2D.Double(this.xEnd, this.yEnd, this.xEnd + dx, this.yEnd + dy), false);
            // Cache shape
