...
public class WizardDisplayerImpl extends WizardDisplayer
{
    ...
    private boolean showInFrame = false;
                       ...
                       Map initialProperties)
    {
        ...
        if (isShowInFrame())
        {
            result = showInFrame(bounds);
        }
        else
        {
            result = showInDialog(bounds);
        }
        return result;
    }
    ...
    protected JModalFrame createFrame()
    {
        ...
        {
            frame = new JModalFrame((Frame) o);
        }
        else if (o instanceof Dialog)
        {
            frame = new JModalFrame((Dialog) o);
        }
        else
        {
            frame = new JModalFrame();
        }
        return frame;
    ...
    protected Object showInFrame(Rectangle bounds)
    {
        JModalFrame frame = createFrame();
        buttonManager.setWindow(frame);
        frame.setTitle(wizard.getTitle());
        ...
        frame.addWindowListener(new WindowAdapter()
        {
            ...
            {
                if (!(e.getWindow() instanceof JModalFrame))
                {
                    return;
                }
                JModalFrame frame = (JModalFrame) e.getWindow();
        ...
        frame.getRootPane().setDefaultButton(buttonManager.getNext());
